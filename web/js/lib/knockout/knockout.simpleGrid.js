(function () {
    ...
    ko.simpleGrid = {
        ...
        viewModel:function (configuration) {
            ...
            self.extendId = configuration.id || "";
            ...
            self.translate = function () {
                ...
                setTimeout(function() {
                    $(".ko-grid").translate();
                    if ($("#pblist" + self.extendId + " tr").length >0 ){
                        $("#ko_grid_checkAll" + self.extendId).prop("disabled", false);
                    }else{
                        $("#ko_grid_checkAll" + self.extendId).prop("disabled", true);
                    }
                }, 100);
            };

            self.clearCheck = function () {
                $("#ko_grid_checkAll" + self.extendId).prop("checked", false);
                $("#pblist" + self.extendId + " :checkbox").prop("checked", false);
                self.checkedCount(0);
                self.translate();
            };
            ...
            self.clearAllChecked = function () {
                var checkAll =  $("#ko_grid_checkAll" + self.extendId);
                checkAll.prop("checked", false);
                checkAll[0].indeterminate = false;
            }

            self.clickCheckAll = function () {
                var checks = $("#pblist" + self.extendId + " :checkbox");
                if($("#ko_grid_checkAll" + self.extendId).is(":checked")) {
                    self.checkedCount(checks.length);
            ...
            self.calCheckAll = function() {
                var checked = $("#pblist" + self.extendId + " :checkbox:checked").length;
                var checkBoxCount = $("#pblist" + self.extendId + " :checkbox").length;
                var checkAll =  $("#ko_grid_checkAll" + self.extendId);
                if(checked == checkBoxCount) {
            ...
            self.selectedItems = function (type) {
                ...
                if (self.tmplType == 'card') {
                    checks = $("#pblist" + self.extendId + " :checkbox:checked");
                }
                else {
                    checks = $("#pblist" + self.extendId + " :checkbox:checked");
                }
            ...
            self.radioSelectValue = function () {
                ...
                return $("#pblist" + self.extendId + " [name='ko_grid_radio']:checked").val();
            }
            ...
            self.radioSelectedPrimaryValue = function () {
                ...
                return $("#pblist" + self.extendId + " [name='ko_grid_radio']:checked").attr("primaryValue");
            }
            ...
            self.clearRadioSelect = function () {
                $("#pblist" + self.extendId + " [name='ko_grid_radio']:checked").removeAttr("checked");
                self.checkedCount(0);
            }

            self.setRadioSelect = function (val) {
                $("#pblist" + self.extendId + " [name='ko_grid_radio'][value='" + val + "']").prop("checked", true);
                self.checkedCount(1);
            }
    ...
    ko.bindingHandlers.simpleGrid = {
        init:function (element, viewModelAccessor) {
            ...
            if (viewModel.tmplType == 'card') {
                ...
                <div class='ko-grid-container table-responsive'>\
                    <div class='ko-grid-menu' data-bind='visible:showMenu'>\
                        <div class='ko-grid-option'>\
                            ...
                            <input class='form-check-input' type='checkbox' id='ko_grid_checkAll' data-bind='visible: tmplType==\"card\"&&data().length>0,click: clickCheckAll,attr:{id: \"ko_grid_checkAll\"+extendId}' />\
                        </div>\
                        <div class='clean'></div>\
                    </div>\
                    <div id=\"pblist\" class=\"ko-grid\" data-bind=\"foreach: itemsOnCurrentPage,attr:{id: 'pblist'+extendId}\">\
                        <div class=\"ko-grid-card\" >\
                            ...
                            <div class=\"ko-grid-card-check\" ><input class='form-check-input' type='checkbox' data-bind='value: $data[$root.idName],attr:{primaryValue:$data[$root.primaryColumn]}'/></div>\
                        </div>\
                     <div>\
                </div>");
            } else {
                ...
                <div class='ko-grid-container table-responsive'>\
                        ...
                        <tr data-bind=\"foreach: columns\" >\
                           <!-- ko if: columnType=='checkbox' -->\
                               <th data-bind='attr:{width:width}, visible: display'>\
                                    <input class='form-check-input' type='checkbox' id='ko_grid_checkAll' data-bind='click:$root.clickCheckAll,attr:{id: \"ko_grid_checkAll\"+$parent.extendId}'/>\
                               </th>\
                           <!-- /ko --> \
                    ...
                    <tbody id=\"pblist\" data-bind=\"foreach: itemsOnCurrentPage,attr:{id: 'pblist'+extendId}\">\
                        <tr data-bind=\"foreach: $parent.columns,css:{'odd': $index() % 2==1,'even':$index()%2==0 } \">\
                            <!-- ko if: columnType=='checkbox' -->\
                            <td class = 'ko-grid-center'><input class='form-check-input' type='checkbox' data-bind='value: $parent[rowText],attr:{primaryValue:$parent[$root.primaryColumn],id:\"chk__\"+ $parent[rowText]},click:function(event){$root.clickCheck($parent[rowText]);return true;}'/></td>\
                            <!-- /ko --> \
                            <!-- ko if: columnType=='radio' -->\
