...
jQuery.validator.addMethod("ssid_for_init", function (value, element, param) {
    return this.optional(element) || (value.indexOf(" ") != 0 && value.lastIndexOf(" ") != (value.length - 1) && /^[0-9a-zA-Z!#\(\)\+\-\.\/%=\?@\^_\{|\}~\x20]{1,32}$/.test(value));
}, "Please enter a valid SSID.");
...
jQuery.validator.addMethod("password_check_for_init", function (value, element, param) {
    return this.optional(element) || value.indexOf(" ") != 0 && value.lastIndexOf(" ") != (value.length - 1) && /^[0-9a-zA-Z!#$\*\+,\-\.%\:\=\?@\[\]\^\_\{|\}~\(\)\/\x20]{1,32}$/.test(value);
}, "Please enter a valid password.");
...
jQuery.validator.addMethod("wifi_password_check", function (value, element, param) {
    return this.optional(element) || value.indexOf(" ") != 0 && value.lastIndexOf(" ") != (value.length - 1) && /^[0-9a-zA-Z!#$\*\+,\-\.%\:\=\?@\[\]\^\_\{|\}~\(\)\/\x20]*$/.test(value);
});
jQuery.validator.addMethod("wifi_password_check_for_init", function (value, element, param) {
    return this.optional(element) || value.indexOf(" ") != 0 && value.lastIndexOf(" ") != (value.length - 1) && /^[0-9a-zA-Z!#$\*\+,\-\.%\:\=\?@\[\]\^\_\{|\}~\(\)\/\x20]*$/.test(value);
});
...
jQuery.validator.addMethod("url_check", function(value, element, param){
	 var strRegex = "^((https|http|ftp|rtsp|mqtt|mms)?://)"       
...
jQuery.validator.addMethod("url_ip_check", function(value, element, param){
...
jQuery.validator.addMethod("ddns_account_check", function (value, element, param) {
    return this.optional(element) || (/^[^";\\]*$/.test(value) && value.indexOf("($)") == -1);
});

jQuery.validator.addMethod("domain_name_charactor_check", function(value, element, param){
    return this.optional(element) || /^[0-9a-zA-Z-\.\*]*$/.test(value);	
});

jQuery.validator.addMethod("domain_name_check", function(value, element, param){
    var strRegex1 = "(([0-9a-zA-Z]+\\.){0,}" // 域名- www.      
                    + "([0-9a-zA-Z][0-9a-zA-Z-]{0,61})?[0-9a-zA-Z]\\." // 二级域名      
                    + "[a-zA-Z]{2,6})"; // first level domain- .com or .museum      
    var strRegex2 = "(^(\\*\\.)" + "([0-9a-zA-Z][0-9a-zA-Z-]{0,61})?[0-9a-zA-Z]" + "(\\.[a-zA-Z]{2,6})*)"
                    +"|"
                    +"(^([0-9a-zA-Z]+\\.)*" + "([0-9a-zA-Z][0-9a-zA-Z-]{0,61})?[0-9a-zA-Z]" + "(\\.\\*)$)"
                    +"|"
                    +"(^(\\*\\.)" + "([0-9a-zA-Z][0-9a-zA-Z-]{0,61})?[0-9a-zA-Z]" + "(\\.[a-zA-Z]{2,6})*" + "(\\.\\*)$)";
    var strRegex3 = "(^(\\*\\.)[0-9a-zA-Z-]+(\\.[a-zA-Z]{2,6})*$)"
                    +"|"
    ...
    if(value.indexOf("*") == -1){
        ...
        return this.optional(element) || (re2.test(value) && re3.test(value));
    }

	
});

jQuery.validator.addClassRules('domainNameCheck', {
    domain_name_charactor_check: true,
    domain_name_check: true
});

jQuery.validator.addClassRules('checkWepPassword', {
    wifi_wep_password_check: true
});

jQuery.validator.addClassRules('checkWifiPassword', {
    wifi_password_check: true
});

jQuery.validator.addMethod("valan_id_check", function (value, element, param) {
    return this.optional(element) || (/^[1-9]\d*$/.test(value) && parseInt(value, 10) >= 1 && parseInt(value, 10) <= 4094);
});

jQuery.validator.addMethod("gre_port", function (value, element, param) {
    return this.optional(element) || value == 8 || value == 16 || value == 24;
});
