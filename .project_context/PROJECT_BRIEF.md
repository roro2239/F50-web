...
已按当前需求移除免密登录功能，后端接口、登录页按钮和插件开关已删除，项目恢复为仅支持原有密码登录链路。根地址接管与恢复闭环已落地：启动脚本负责接管 `http://192.168.0.1/` 到当前后台，停止脚本负责恢复 `http://192.168.0.1:8080/` 的原厂后台并尝试拉起 `com.zte.web`，卸载脚本进一步补齐 `iptables`/`ip6tables`、ADB 端口、`device_config`、`settings`、下载临时文件、自启脚本和 `UFI-TOOLS` 插件存储的清理；插件自身的卸载按钮也改为通过 `set_custom_head` 正式注销当前插件块，避免仅靠 root 清理应用私有目录。登录页已新增官方后台入口，前端新增 F50 设备识别并在首页、无卡页和菜单层隐藏电量展示与 NFC 入口；对应 Android arm64 二进制已重新编译并部署到设备，`http://192.168.0.1/` 与 `http://192.168.0.1:8080/` 均返回 `200`，设备端已回查到新的登录模板和前端脚本内容。下一步只剩真实执行卸载后的残留回滚验证。
