...
已按当前需求移除免密登录功能，后端接口、登录页按钮和插件开关已删除，项目恢复为仅支持原有密码登录链路。当前正在补齐“根地址接管”和“卸载恢复”闭环：启动脚本负责接管 `http://192.168.0.1/` 到当前后台，停止/卸载脚本负责恢复 `http://192.168.0.1:8080/` 的原厂后台并尝试拉起 `com.zte.web`；插件已新增卸载入口。下一步是重新编译并部署设备，验证根地址访问与卸载恢复行为。
